...
- [ ] Annotation appearance streams — form fields, stamps and comments are not
      painted. Whether this matters depends on the intended reader.
...
- [ ] **Rotate the exposed OTEL token.** The repository is public.
      `.env.example` carries placeholders only, but a token that was once
      committed stays live until it is rotated at the collector.
- [ ] **Branch protection on `master`.** There is none.
- [ ] **Is a FIPS-capable build a product requirement?** It changes the crypto
      dependency and the packaging story.
- [ ] **npm provenance signing.** Cheap at publish time; a supply-chain claim to
      make or not make.
