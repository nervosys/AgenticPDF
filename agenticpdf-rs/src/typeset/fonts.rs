...
impl Family {
    ...
    pub fn from_name(name: &str) -> Family {
        ...
        if ["times", "roman", "georgia", "garamond", "cambria", "book", "serif"]
            .iter()
            .any(|needle| lower.contains(needle))
            && !lower.contains("sans")
        {
        ...
        if ["helvetica", "arial", "calibri", "verdana", "tahoma", "segoe", "sans"]
            .iter()
            .any(|needle| lower.contains(needle))
        {
...
mod tests {
    ...
    fn resolves_a_style_to_a_postscript_name() {
        ...
        assert_eq!(Font::resolve(&italic).postscript_name(), "Helvetica-Oblique");
