...
in `irondoc.h` matches the `#[unsafe(no_mangle)] extern "C"` functions it declares.
...
wraps all of them with a `defer { irondoc_string_free(...) }`, so no call site has
