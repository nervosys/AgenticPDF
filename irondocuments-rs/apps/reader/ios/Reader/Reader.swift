...
enum Reader {
    ...
    /// can leak by forgetting `irondoc_string_free`.
    private static func take(_ pointer: UnsafeMutablePointer<CChar>?) -> String {
        guard let pointer else { return "{}" }
        defer { irondoc_string_free(pointer) }
    ...
    static func open(_ data: Data) -> [String: Any] {
        let text = data.withUnsafeBytes { buffer in
            take(irondoc_open(buffer.bindMemory(to: UInt8.self).baseAddress, data.count))
        }
        return json(text)
    }
    ...
    static func execute(_ action: String, _ params: [String: Any] = [:]) -> [String: Any] {
        ...
        return json(take(irondoc_execute(action, String(decoding: encoded, as: UTF8.self))))
    }
    ...
    static func renderPage(width: CGFloat, height: CGFloat, zoom: CGFloat) -> [[String: Any]] {
        let text = take(irondoc_render_page(Float(width), Float(height), Float(zoom)))
        return ((try? JSONSerialization.jsonObject(with: Data(text.utf8))) as? [[String: Any]]) ?? []
    }

    /// Serialise to ADF. Two calls: ask the size, allocate, fill.
    static func save() -> Data {
        let size = irondoc_save(nil, 0)
        guard size > 0 else { return Data() }
        var buffer = [UInt8](repeating: 0, count: size)
        _ = buffer.withUnsafeMutableBufferPointer { irondoc_save($0.baseAddress, size) }
        return Data(buffer)
    }

    static var isDirty: Bool { irondoc_is_dirty() }

    static func capabilities() -> [String: Any] { json(take(irondoc_capabilities())) }
}
