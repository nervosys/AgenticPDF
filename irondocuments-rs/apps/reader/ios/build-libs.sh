...
for target in aarch64-apple-ios aarch64-apple-ios-sim x86_64-apple-ios; do
  ...
  (cd "$CRATE_ROOT" && cargo build --release -p irondoc-reader --lib --target "$target")
done
...
lipo -create \
  "$CRATE_ROOT/target/aarch64-apple-ios-sim/release/libirondoc_reader.a" \
  "$CRATE_ROOT/target/x86_64-apple-ios/release/libirondoc_reader.a" \
  -output "$OUT/libirondoc_reader_sim.a"

rm -rf "$OUT/APDFReader.xcframework"
xcodebuild -create-xcframework \
  -library "$CRATE_ROOT/target/aarch64-apple-ios/release/libirondoc_reader.a" \
  -headers "$(dirname "$0")/Reader/irondoc.h" \
  -library "$OUT/libirondoc_reader_sim.a" \
  -headers "$(dirname "$0")/Reader/irondoc.h" \
