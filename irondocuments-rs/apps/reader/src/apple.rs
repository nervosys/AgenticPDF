...
//! [`irondoc_string_free`], and `Reader.swift` wraps each one so a caller cannot
...
/// Hand a string to the caller, who must free it with [`irondoc_string_free`].
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn irondoc_open(data: *const u8, len: usize) -> *mut c_char {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn irondoc_execute(
    action: *const c_char,
    params: *const c_char,
) -> *mut c_char {
    catch_unwind(AssertUnwindSafe(|| {
...
#[unsafe(no_mangle)]
pub extern "C" fn irondoc_render_page(width: f32, height: f32, zoom: f32) -> *mut c_char {
    catch_unwind(|| {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn irondoc_save(out: *mut u8, capacity: usize) -> usize {
    catch_unwind(AssertUnwindSafe(|| {
        let Ok(mut guard) = SESSION.lock() else {
            return 0;
        };
...
#[unsafe(no_mangle)]
pub extern "C" fn irondoc_is_dirty() -> bool {
    SESSION
        .lock()
...
#[unsafe(no_mangle)]
pub extern "C" fn irondoc_capabilities() -> *mut c_char {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn irondoc_string_free(pointer: *mut c_char) {
    if !pointer.is_null() {
