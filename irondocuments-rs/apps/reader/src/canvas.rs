...
use irondocuments::engine::{DisplayList, RenderOp};
...
fn content_key(pixels: &[u8]) -> String {
    ...
    if pixels.len() <= WHOLE {
        let digest = irondocuments::adf::sha256::sha256(pixels);
        return irondocuments::adf::sha256::hex(&digest[..8]);
    }
    ...
    let digest = irondocuments::adf::sha256::sha256(&probe);
    irondocuments::adf::sha256::hex(&digest[..8])
}
    ...
    fn draw_image(&mut self, rect: Rect, image: &ImageData<'_>) {
        ...
        let pixels = match &key {
            ...
            Some(key) if self.images.insert(key.clone()) => {
                format!(r#","pixels":"{}""#, irondocuments::engine::b64e(pixels_ref))
            }
            _ => String::new(),
        };
        let key = match &key {
...
mod render_report {
    ...
    fn textures_decoded_against_drawn() {
        ...
        let list =
            irondocuments::engine::extract_display_list(&bytes, page).expect("a display list");
        ...
        let decoded =
            irondocuments::engine::extract_page_textures(&bytes, page).unwrap_or_default();
...
/BitsPerComponent 1/Length 1>>stream\n",
        ...
        irondocuments::engine::reset_soft_mask_census();
        let textures = irondocuments::engine::extract_page_textures(&pdf, 1).unwrap_or_default();
        let census = irondocuments::engine::soft_mask_census();
        assert_eq!(
    ...
    fn flat_textures() {
        ...
        > = list
            ...
            .collect();
        for t in irondocuments::engine::extract_page_textures(&bytes, page).unwrap_or_default() {
            let n = t.rgba.len() / 4;
            if n == 0 {
    ...
    /// -p irondoc-reader -- --ignored what_the_corpus_declines --nocapture`
    #[test]
    ...
    fn what_the_corpus_declines() {
        ...
        let c = irondocuments::engine::soft_mask_census();
        eprintln!(
    ...
    fn soft_masks_reached() {
        ...
        for path in &documents {
            ...
            let now = irondocuments::engine::soft_mask_census();
