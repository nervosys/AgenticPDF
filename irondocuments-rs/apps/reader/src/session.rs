...
use irondocuments::adf::oplog::{Actor, Change, OpId, OpLog};
use irondocuments::adf::{AdfDoc, AdfWriter};
use irondocuments::detect::Format;
use irondocuments::doc::{Block, SemanticDoc};
use irondocuments::document::Document;
use irondocuments::{PdfError, engine::DisplayList};
...
impl Session {
    ...
    pub fn open(bytes: Vec<u8>) -> Result<Session, PdfError> {
        ...
        let log = match document.format() {
            ...
            _ => {
                ...
                let mut log = irondocuments::adf::oplog::from_blocks(&blocks, ACTOR_USER, 0);
    ...
    pub fn textures(&self, budget_pixels: usize) -> Arc<Vec<crate::canvas::Texture>> {
        ...
        let decoded: Vec<crate::canvas::Texture> =
            irondocuments::engine::extract_page_textures(&self.source, self.page)
                .unwrap_or_default()
                .into_iter()
                .map(|t| {
    ...
    fn scan(&self, query: &str) -> Vec<Hit> {
        ...
        for (section_index, section) in semantic.sections.iter().enumerate() {
            for (block_index, block) in section.blocks.iter().enumerate() {
                ...
                irondocuments::doc::block_text_into(block, &mut text);
    ...
    pub fn save(&mut self) -> Result<Vec<u8>, PdfError> {
        let bytes = if self.document.format() == Format::Adf {
            ...
            irondocuments::adf::write::append_ops(&mut file, &self.log, self.saved_mark);
            file
        } else {
    ...
    pub fn materialized(&self) -> SemanticDoc {
        ...
        match semantic.sections.first_mut() {
            ...
            None => semantic.sections.push(irondocuments::doc::Section {
                blocks,
                ..irondocuments::doc::Section::default()
            }),
        }
        semantic
    }
    ...
    pub fn export(&self, to: &str) -> Result<String, PdfError> {
        ...
        Ok(match to.to_ascii_lowercase().as_str() {
            "markdown" | "md" => irondocuments::doc::to_markdown(&semantic),
            "html" => irondocuments::doc::to_html(&semantic),
...
mod tests {
    ...
    fn editing_marks_the_session_dirty_and_shows_in_the_blocks() {
        ...
        let mut text = String::new();
        irondocuments::doc::block_text_into(&blocks[3].1, &mut text);
