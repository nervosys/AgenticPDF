...
// `Cell` is already a table cell here, so the spreadsheet one is aliased.
...
// Spreadsheet chunks
//
// A workbook is split the same way a report is: a directory chunk that names
// the parts, and one chunk per part. Listing the sheets in a 200 MB workbook
// reads the directory and nothing else, and opening one sheet decodes one
// chunk — the same "cost of a question is the size of its answer" property the
// section chunks give prose.
// ---------------------------------------------------------------------------

/// Encode the workbook directory: sheet names and defined names.
///
/// Sheet names are repeated here rather than only in the sheet chunks so that
/// listing a workbook does not have to open, and possibly decompress, every
/// sheet it contains.
pub fn encode_workbook(workbook: &Workbook, table: &mut StringTable) -> Vec<u8> {
    let mut out = Writer::new();

    out.usize(workbook.sheets.len());
    for sheet in &workbook.sheets {
        out.u32(table.intern(&sheet.name));
    }

    out.usize(workbook.defined_names.len());
    for name in &workbook.defined_names {
        out.u32(table.intern(&name.name));
        out.u32(table.intern(&name.refers_to));
        out.option(name.sheet, |w, index| w.u32(index));
    }

    out.bytes
}

/// The workbook directory, as `(sheet names, defined names)`.
pub fn decode_workbook(
    bytes: &[u8],
    heap: &StringHeap<'_>,
) -> Result<(Vec<String>, Vec<DefinedName>), AdfError> {
    let mut reader = Reader::new(bytes);

    let sheet_count = reader.count()?;
    let mut names = Vec::with_capacity(sheet_count);
    for _ in 0..sheet_count {
        names.push(heap.owned(reader.u32()?)?);
    }

    let defined_count = reader.count()?;
    let mut defined = Vec::with_capacity(defined_count);
    for _ in 0..defined_count {
        defined.push(DefinedName {
            name: heap.owned(reader.u32()?)?,
            refers_to: heap.owned(reader.u32()?)?,
            sheet: reader.option(|r| r.u32())?,
        });
    }
...
/// Encode one worksheet.
pub fn encode_sheet(sheet: &Sheet, table: &mut StringTable) -> Vec<u8> {
    let mut out = Writer::new();

    out.u32(table.intern(&sheet.name));
    out.u8(u8::from(sheet.hidden));
    out.option(sheet.frozen, |w, (rows, columns)| {
        w.u32(rows);
        w.u32(columns);
    });

    out.usize(sheet.hidden_rows.len());
    for row in &sheet.hidden_rows {
        out.varint(*row as u64);
    }
    out.usize(sheet.hidden_columns.len());
    for column in &sheet.hidden_columns {
        out.varint(*column as u64);
    }

    out.usize(sheet.column_widths.len());
    for width in &sheet.column_widths {
        out.f64(*width);
    }

    out.usize(sheet.merges.len());
    for merge in &sheet.merges {
        out.varint(merge.first_row as u64);
        out.varint(merge.first_column as u64);
        out.varint(merge.last_row as u64);
        out.varint(merge.last_column as u64);
    }

    out.usize(sheet.cells.len());
    for cell in &sheet.cells {
        out.varint(cell.row as u64);
        out.varint(cell.column as u64);

        // One flag byte rather than three option tags: the overwhelming
        // majority of cells carry none of these, and a byte each would be a
        // third of the size of a plain numeric cell.
        let mut flags = 0u8;
        ...
        if cell.href.is_some() {
            flags |= 1 << 2;
        }
        out.u8(flags);

        match &cell.value {
            Value::Empty => out.u8(0),
            Value::Number(number) => {
                out.u8(1);
                out.f64(*number);
            }
...
/// Decode a worksheet written by [`encode_sheet`].
pub fn decode_sheet(bytes: &[u8], heap: &StringHeap<'_>) -> Result<Sheet, AdfError> {
    let mut reader = Reader::new(bytes);

    let name = heap.owned(reader.u32()?)?;
    let hidden = reader.u8()? != 0;
    let frozen = reader.option(|r| Ok((r.u32()?, r.u32()?)))?;

    let hidden_row_count = reader.count()?;
    let mut hidden_rows = Vec::with_capacity(hidden_row_count);
    for _ in 0..hidden_row_count {
        hidden_rows.push(read_index(&mut reader)?);
    }
    let hidden_column_count = reader.count()?;
    let mut hidden_columns = Vec::with_capacity(hidden_column_count);
    for _ in 0..hidden_column_count {
        hidden_columns.push(read_index(&mut reader)?);
    }

    let width_count = reader.count()?;
    let mut column_widths = Vec::with_capacity(width_count);
    for _ in 0..width_count {
        column_widths.push(reader.f64()?);
    }

    let merge_count = reader.count()?;
    let mut merges = Vec::with_capacity(merge_count);
    for _ in 0..merge_count {
        merges.push(Range {
            first_row: read_index(&mut reader)?,
            first_column: read_index(&mut reader)?,
            last_row: read_index(&mut reader)?,
            last_column: read_index(&mut reader)?,
        });
    }
    ...
    for _ in 0..cell_count {
        let row = read_index(&mut reader)?;
        let column = read_index(&mut reader)?;
        let flags = reader.u8()?;

        let value = match reader.u8()? {
            0 => Value::Empty,
            1 => Value::Number(reader.f64()?),
            2 => Value::Text(heap.owned(reader.u32()?)?),
            3 => Value::Bool(reader.u8()? != 0),
            4 => Value::Error(
                CellError::from_u8(reader.u8()?)
                    .ok_or(AdfError::Malformed("unknown cell error code"))?,
            ),
            _ => return Err(AdfError::Malformed("unknown cell value tag")),
        };

        let mut optional = [None, None, None];
        for (bit, slot) in optional.iter_mut().enumerate() {
            if flags & (1 << bit) != 0 {
                *slot = Some(heap.owned(reader.u32()?)?);
            }
        }
        let [formula, number_format, href] = optional;
    ...
    // The reader relies on cells being sorted — `Sheet::cell` binary searches
    // them. A file that arrives out of order is malformed rather than merely
    // unusual, and silently sorting it would hide a broken writer.
    if cells
        .windows(2)
        .any(|pair| (pair[0].row, pair[0].column) >= (pair[1].row, pair[1].column))
    {
        return Err(AdfError::Malformed(
            "sheet cells are not in ascending order",
        ));
    }
...
/// A row or column index, which is a `u32` on disk written as a varint.
fn read_index(reader: &mut Reader<'_>) -> Result<u32, AdfError> {
    u32::try_from(reader.varint()?).map_err(|_| AdfError::Malformed("row or column out of range"))
}
