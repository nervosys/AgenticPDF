...
use crate::sheet::{DefinedName, Sheet, Workbook};
...
use super::{AdfError, ChunkEntry, ChunkKind, ENTRY_LEN, Header, Profile};
...
impl<'a> AdfDoc<'a> {
    ...
    /// Which kind of document this file holds.
    ///
    /// Reading it costs the header and nothing else, so a caller can pick a
    /// decoder before deciding whether to pay for any chunk.
    pub fn profile(&self) -> Profile {
        self.header.profile
    }

    /// Number of worksheets, without decoding any of them.
    pub fn sheet_count(&self) -> usize {
        self.count_of(ChunkKind::Sheet)
    }

    /// The sheet names and defined names, without decoding any sheet.
    ///
    /// Empty for a file that holds no workbook directory, which is every file
    /// written under the document profile.
    pub fn workbook_directory(&self) -> Result<(Vec<String>, Vec<DefinedName>), AdfError> {
        let Some(bytes) = self.chunk(ChunkKind::Workbook, 0) else {
            return Ok((Vec::new(), Vec::new()));
        };
        codec::decode_workbook(bytes, &self.strings()?)
    }

    /// Decode one worksheet. The spreadsheet counterpart of [`Self::section`],
    /// and cheap for the same reason: header, table, heap, one chunk.
    pub fn sheet(&self, index: u32) -> Result<Sheet, AdfError> {
        let heap = self.strings()?;
        let bytes = self
            .chunk(ChunkKind::Sheet, index)
            .ok_or(AdfError::MissingChunk(ChunkKind::Sheet))?;
        codec::decode_sheet(bytes, &heap)
    }

    /// The whole workbook, every sheet decoded.
    pub fn workbook(&self) -> Result<Workbook, AdfError> {
        let (_, defined_names) = self.workbook_directory()?;
        let mut sheets = Vec::with_capacity(self.sheet_count());
        for index in 0..self.sheet_count() as u32 {
            sheets.push(self.sheet(index)?);
        }
        Ok(Workbook {
            sheets,
            defined_names,
    ...
    pub fn to_semantic(&self) -> Result<SemanticDoc, AdfError> {
        ...
        // A workbook stores cells, not sections. Rendering them here is what
        // lets Markdown output, chunking and typesetting work on a spreadsheet
        // without any of them knowing the profile exists. Sections written
        // alongside sheets, if a future writer emits both, are kept ahead of
        // the rendered ones rather than replaced by them.
        for index in 0..self.sheet_count() as u32 {
            let sheet = self.sheet(index)?;
            // A sheet the author hid stays in the file and out of the text,
            // exactly as the readers of the source formats have always
            // treated one. It is reachable through `sheet()` and `workbook()`,
            // which are asking a different question.
            if sheet.hidden {
                continue;
            }
            document.sections.push(sheet.to_section());
        }
