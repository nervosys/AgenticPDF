...
use super::{ChunkEntry, ChunkKind, Header, Profile, VERSION_MAJOR, VERSION_MINOR, chunk_flags};
...
pub struct AdfWriter {
    ...
    doc_id: [u8; 16],
    compress: bool,
    /// Set by whichever `write_*` entry point is used, so a caller cannot
    /// label a workbook as prose by forgetting a builder call.
    profile: Profile,
}
...
impl AdfWriter {
    ...
    /// Encode a workbook, building a retrieval index over its rows.
    ///
    /// `metadata` supplies only the document-level fields — title, author,
    /// dates — which a workbook has as surely as a report does but which the
    /// sheets themselves do not carry.
    pub fn write_workbook(
        mut self,
        workbook: &Workbook,
        metadata: &SemanticDoc,
        source_format: &str,
    ) -> Vec<u8> {
        // Set here rather than by a builder call the caller might forget: a
        // file whose header says "prose" and whose chunks are sheets would be
        // readable only by accident.
        self.profile = Profile::Spreadsheet;

        let meta = codec::encode_meta(metadata, source_format, &mut self.strings);
        self.chunks.push(PendingChunk {
            kind: ChunkKind::Meta,
            id: 0,
            payload: meta,
        ...
        let directory = codec::encode_workbook(workbook, &mut self.strings);
        self.chunks.push(PendingChunk {
            kind: ChunkKind::Workbook,
            id: 0,
            payload: directory,
        });

        for (index, sheet) in workbook.sheets.iter().enumerate() {
            let payload = codec::encode_sheet(sheet, &mut self.strings);
            self.chunks.push(PendingChunk {
                kind: ChunkKind::Sheet,
                id: index as u32,
                payload,
            });
            self.index_sheet(index as u32, sheet);
        }
    ...
    /// Split a sheet into retrieval chunks, one per used row.
    ///
    /// A row rather than a cell: `1783.5` on its own is not searchable text and
    /// not a citable answer, while the row it sits in carries its label and the
    /// figures beside it. `blocks` holds the row index, so a hit points back at
    /// a sheet and a row rather than at the workbook as a whole.
    fn index_sheet(&mut self, sheet: u32, worksheet: &crate::sheet::Sheet) {
        for (row, cells) in worksheet.rows() {
