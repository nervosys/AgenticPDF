...
impl SemanticDoc {
    ...
    /// Project onto the engine's logical-structure tree, so `irondoc structure`
...
pub fn inlines_are_empty(content: &[Inline]) -> bool {
    inline_text(content).trim().is_empty()
        && !content
            .iter()
            .any(|inline| matches!(inline, Inline::Image(_) | Inline::FootnoteRef { .. }))
}
...
fn render_run(run: &Run) -> String {
    ...
    // would hide an injection payload from a reviewer. `irondoc scan` reports it,
