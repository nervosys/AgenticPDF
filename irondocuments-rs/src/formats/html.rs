...
pub fn parse_html_with_styles(data: &[u8], linked: &[String]) -> SemanticDoc {
    ...
    for css in linked {
        ...
        linked_rules
            .classes
            .extend(std::mem::take(&mut sheet.classes));
    ...
    let loose = blocks
        .iter()
        .enumerate()
        .any(|(at, block)| !inside[at] && !matches!(block, Block::Divider | Block::PageBreak));
...
impl Parser {
            ...
            match self.tokens[self.at].clone() {
                ...
                Token::Open { name, attrs } => {
                    ...
                    match name.as_str() {
                        ...
                        "ol" if attribute(&attrs, "class").is_some_and(|class| {
                            class.split_whitespace().any(|c| c == "footnotes")
                        }) =>
                        {
                            flush!();
                            self.at += 1;
                        ...
                        "div" | "article" | "main" | "body" | "html" | "header" | "footer"
    ...
    fn footnote_reference(&mut self) -> Option<usize> {
        ...
        let number: usize = attribute(attrs, "href")?
            .strip_prefix("#fn")?
            .parse()
            .ok()?;
...
mod tests {
    ...
    fn word_quote_class_becomes_a_quotation() {
        ...
        assert_eq!(
            markdown_of("<p class=MsoIntenseQuote>Loud.</p>"),
            "> Loud.\n"
        );
