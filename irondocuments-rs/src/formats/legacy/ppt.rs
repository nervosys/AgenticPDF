...
use crate::doc::{
    Align, Block, Cell, ImageRef, Inline, ListItem, Row, Run, Section, SectionKind, SemanticDoc,
    Table, TextStyle,
};
...
fn layout<'a>(stream: &'a [u8], current_user: &[u8]) -> Option<Layout<'a>> {
    ...
    let is_document = |offset: usize| matches!(record_at(stream, offset), Some((record, _)) if record.kind == RT_DOCUMENT);
    let named = document_id.and_then(|id| persist.get(&id)).copied();
    let document_at = named.filter(|&offset| is_document(offset)).or_else(|| {
        persist
            .values()
            .copied()
            .find(|&offset| is_document(offset))
    })?;
        ...
        for record in children(data) {
            ...
            *budget -= 1;
            match record.kind {
                ...
                _ if record.is_container() => walk(record.body, depth + 1, budget, sections, links),
                _ => {}
            }
        }
    }
...
fn shape_picture(body: &[u8]) -> Option<(usize, Option<String>)> {
    let is_picture = children(body)
        .any(|record| record.kind == ART_FSP && record.instance() == SHAPE_TYPE_PICTURE);
    if !is_picture {
        return None;
    }
...
fn shape_hyperlink(body: &[u8], targets: &HashMap<u32, String>) -> Option<(u32, u32, String)> {
    ...
    fn walk(data: &[u8], depth: usize, id: &mut Option<u32>, range: &mut Option<(u32, u32)>) {
        if depth > MAX_DEPTH {
            return;
        }
        for record in children(data) {
...
mod tests {
    ...
    fn reads_outline_levels_and_character_formatting() {
        ...
        let section = build_slide(
            collect_shapes(&container(RT_SLIDE, &body), &HashMap::new()),
            Vec::new(),
        );
        let mut document = SemanticDoc::default();
