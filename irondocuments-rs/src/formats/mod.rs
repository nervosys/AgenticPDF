...
/// Parse a spreadsheet into the cell model, keeping types and formulas.
///
/// Only formats that actually have cells are accepted. A report is not a
/// workbook with one sheet, and pretending otherwise would put an empty grid
/// in front of a caller who asked a question about a document.
pub fn parse_workbook(
    data: &[u8],
    format: Format,
) -> Result<(crate::sheet::Workbook, SemanticDoc), PdfError> {
    match format {
        Format::Xlsx => ooxml::parse_workbook(data, format),
        other => Err(PdfError::Unsupported(format!(
            "{} has no cell model yet; read it as a document instead",
            other.label()
        ))),
    }
}
