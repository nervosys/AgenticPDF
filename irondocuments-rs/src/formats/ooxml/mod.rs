...
/// Parse a spreadsheet package into the cell model.
///
/// Returns the workbook and, separately, a document carrying only the core
/// properties: a workbook has a title and an author, but they live in the
/// package rather than in any sheet.
pub fn parse_workbook(
    ...
    let workbook = match format {
        Format::Xlsx => xlsx::parse_workbook(&archive, &package)?,
        other => {
            return Err(PdfError::Unsupported(format!(
    ...
    let mut metadata = SemanticDoc::default();
    package.apply_core_properties(&mut metadata);
