...
impl<'a> Parser<'a> {
            ...
            match self.data[self.at] {
                ...
                b'}' => {
                    ...
                    if self
                        .notes
                        .last()
                        .is_some_and(|n| n.depth == self.stack.len())
                    {
                        self.finish_note();
                    }
                    ...
                    if self
                        .field
                        .as_ref()
                        .is_some_and(|f| f.depth == self.stack.len())
                    {
                        self.finish_field();
                    }
    ...
    fn at_style_definition(&self) -> bool {
        self.state.destination == Destination::StyleSheet
            && self
                .style_depth
                .is_some_and(|depth| self.stack.len() == depth + 1)
    }
    ...
    fn end_paragraph_inner(&mut self) -> bool {
        ...
        if is_list {
            let ordered = marker.as_deref().is_some_and(is_ordered_marker);
            let item = ListItem {
                blocks: vec![block],
                checked: None,
            };
...
mod tests {
    ...
    fn lettered_and_roman_markers_produce_an_ordered_list() {
        ...
        for (marker, expected) in [
            ...
            let rtf = format!(r"{HEADER}\pard{{\listtext {marker}\tab}}\ilvl0 Item\par}}");
    ...
    fn a_field_result_is_not_split_after_its_first_character() {
        let rtf =
            format!(r"{HEADER}\pard {{\field{{\*\fldinst PAGE }}{{\fldrslt \b Error!}}}}\par}}");
    ...
    fn reads_a_picture_out_of_its_hex() {
        let rtf = format!(r"{HEADER}\pard {{\pict\pngblip\picwgoal1440\pichgoal720 {PNG}}}\par}}");
        let document = parse_rtf(rtf.as_bytes());
    ...
    fn leaves_a_metafile_picture_alone() {
        let rtf =
            format!(r"{HEADER}\pard {{\pict\wmetafile8\picwgoal1440 0102030405060708}}\par}}");
        let document = parse_rtf(rtf.as_bytes());
    ...
    fn tracked_deletions_are_not_shown() {
        let rtf = format!(r"{HEADER}\pard The {{\deleted old}}{{\revised new}} wording.\par}}");
    ...
    fn a_font_without_a_charset_keeps_the_default_decoding() {
        let rtf =
            format!(r"{HEADER}{{\fonttbl{{\f0 Calibri;}}}}\pard\f0 caf\'e9 \'93quoted\'94\par}}");
    ...
    fn reads_colours_through_the_colour_table() {
        ...
        assert_eq!(
            colours,
            vec![Some([1.0, 1.0, 1.0]), Some([1.0, 0.0, 0.0]), None,]
        );
    }
    ...
    fn the_colour_table_is_not_emitted_as_prose() {
        let rtf = format!(r"{HEADER}{{\colortbl;\red255\green255\blue255;}}\pard\cf1 text\par}}");
        assert_eq!(markdown_of(&rtf), "text\n");
    }
