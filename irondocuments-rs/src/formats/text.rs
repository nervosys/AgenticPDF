...
fn take_footnotes<'a>(lines: &[&'a str]) -> (Vec<&'a str>, Notes) {
    ...
    for line in lines {
        ...
        // document again.
        let continues =
            open.is_some() && line.starts_with(&[' ', '\t'][..]) && !line.trim().is_empty();
        match continues {
            true => {
                if let Some(at) = open
    ...
    while at < lines.len() {
        ...
        if trimmed.starts_with('>') {
            ...
            blocks.push(Block::Quote {
                blocks: parse_blocks(&inner, depth + 1, notes),
            });
            continue;
        }
...
mod tests {
    ...
    fn markdown_reads_the_inline_html_it_writes() {
        ...
        assert!(
            passed.text().contains("<span class=x>"),
            "{}",
            passed.text()
        );
    }
