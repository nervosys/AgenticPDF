...
//! can discover and invoke programmatically. Run `irondoc describe` to get a
//! machine-readable JSON-LD ontology of all commands, parameters, output
//! schemas, and workflows.
//!
//! Usage:
//!   irondoc text        <file>  [--pages 1-5] [--format json|text] [--output path]
//!   irondoc meta        <file>  [--format json|text]
//!   irondoc annotations <file>  [--pages 1-5] [--format json|text]
//!   irondoc outline     <file>  [--format json|text]
//!   irondoc images      <file>  [--pages 1-5] [--format json|text]
//!   irondoc chunk       <file>  [--size 500] [--overlap 50] [--format json|text]
//!   irondoc all         <file>  [--chunk-size 500] [--chunk-overlap 50] [--output path]
//!   irondoc describe
//!   irondoc info        [--format json|text]

#[cfg(feature = "cli")]
use clap::{Parser, Subcommand};

#[cfg(feature = "cli")]
use irondocuments::{PdfError, document::Document};
#[cfg(feature = "cli")]
use std::fs;
use std::process;
...
#[command(
    ...
    long_about = "IronDocuments (irondoc) — read a document and get structure back: text, headings, tables, figures, metadata, annotations and semantic chunks, with JSON output throughout.\n\nReads PDF, Word, Excel and PowerPoint (modern and legacy), OpenDocument, EPUB, HTML, Markdown, CSV, RTF, plain text and ADF. Every command below takes any of them; run `irondoc formats` for the list this build supports.\n\nRun `irondoc describe` for a machine-readable JSON-LD ontology, or `irondoc mcp` to serve the same operations over the Model Context Protocol."
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
enum Commands {
    ...
    /// Read a spreadsheet as typed cells, with formulas (spreadsheets only)
    Sheet {
        /// Path to the spreadsheet (.xlsx, or .adf under the spreadsheet profile)
        file: String,
        /// Only this sheet, by name or by zero-based index
        #[arg(short, long)]
        sheet: Option<String>,
        /// Output format: text or json
        #[arg(short, long, default_value = "text")]
    ...
    /// Recompute a spreadsheet's formulas and report what disagrees (spreadsheets only)
    Recalc {
        /// Path to the spreadsheet (.xlsx, or .adf under the spreadsheet profile)
        file: String,
        /// Output format: text or json
    ...
    #[cfg(feature = "cli")]
    {
        ...
        let result = match cli.command {
            ...
            Commands::Sheet {
                ...
                format,
                output,
            } => cmd_sheet(&file, sheet.as_deref(), &format, output.as_deref()),
            ...
            Commands::Mcp => irondocuments::mcp::serve().map_err(PdfError::from),
...
fn cmd_table(
    ...
    let result = match format {
        "json" => serde_json::to_string_pretty(&tables)
            .map_err(|e| PdfError::ExportError(e.to_string()))?,
        _ => irondocuments::tables::to_markdown(&tables),
    };
    match output {
        Some(path) => fs::write(path, &result)?,
        None => println!("{}", result),
...
fn cmd_scanned(file: &str, format: &str) -> Result<(), PdfError> {
    ...
    let report = irondocuments::ocr::detect_scanned(data, doc)?;
    ...
    match format {
        ...
        _ => {
            if scanned.is_empty() {
                ...
                if !irondocuments::ocr::OCR_BUILTIN {
                    println!(
...
fn cmd_ocr(
    ...
    let results = match (vlm, server) {
        (Some(url), _) => irondocuments::ocr::recognize_scanned_vlm(data, doc, url, model)?,
        (None, Some(url)) => irondocuments::ocr::recognize_scanned_http(data, doc, url)?,
        (None, None) => irondocuments::ocr::recognize_scanned(data, doc, lang)?,
    };
    if results.is_empty() {
...
fn cmd_forms(file: &str, format: &str) -> Result<(), PdfError> {
    ...
    let data = document
        ...
        .map(|_| document.data())?;
    let fields = irondocuments::engine::extract_form_fields(data)?;
    match format {
        "json" => {
    ...
    match format {
        ...
        _ => {
            if tree.is_empty() {
                ...
                fn print_nodes(nodes: &[irondocuments::engine::StructNode], depth: usize) {
                    for n in nodes {
            ...
            if report.clean {
                ...
                for f in &report.findings {
                    let reason = match f.reason {
                        irondocuments::sanitize::Reason::OffPage => "off-page",
                        irondocuments::sanitize::Reason::TinyText => "tiny-text",
                        irondocuments::sanitize::Reason::Hidden => "hidden",
                    };
    ...
    match format {
        ...
        _ => {
            if outline.is_empty() {
                ...
                fn print_items(items: &[irondocuments::OutlineItem], depth: usize) {
                    for item in items {
                        let indent = "  ".repeat(depth);
...
fn cmd_images(file: &str, pages: Option<&str>, format: &str) -> Result<(), PdfError> {
    ...
    let images: Vec<_> = irondocuments::engine::extract_images(data)?
        .into_iter()
...
fn cmd_search(file: &str, query: &str, json: bool) -> Result<(), PdfError> {
    ...
    let document = load_document(file)?;
    let result = irondocuments::agent_ops::search(&data, &document, query)?;
...
fn cmd_verify(
    ...
    let data = fs::read(file)?;
    if !irondocuments::adf::AdfDoc::sniff(&data) {
        return Err(PdfError::Unsupported(
            "only ADF documents carry provenance; import with `convert --to adf` first".into(),
        ));
    }
    let verdict = irondocuments::agent_ops::verify(&data, text, section, block)?;
...
fn cmd_convert(file: &str, to: &str, sanitize: bool, output: Option<&str>) -> Result<(), PdfError> {
    ...
    let result: Vec<u8> = match to.to_ascii_lowercase().as_str() {
        ...
        "adf" => {
            // A spreadsheet imports as cells when this build can read them,
            // because converting a workbook into a table of text and calling
            // it an agentic document format would be a lie. Anything else --
            // including a spreadsheet format with no cell reader yet -- takes
            // the prose path.
            match irondocuments::formats::parse_workbook(document.data(), document.format()) {
                Ok((workbook, metadata)) => irondocuments::agent_ops::write_adf_workbook(
                    &workbook,
                    &metadata,
                    file,
                    document.format(),
                ),
                Err(_) => irondocuments::agent_ops::write_adf(
                    &document.semantic_view(),
                    file,
                    document.format(),
                ),
            }
...
///
/// The other commands answer "what does this say?". This one answers "what is
/// in it?" — the type of every value, the formula behind the computed ones, and
/// the format that says whether a number is a date. Reads `.xlsx` directly and
/// `.adf` written under the spreadsheet profile.
#[cfg(feature = "cli")]
fn cmd_sheet(
    file: &str,
    wanted: Option<&str>,
    ...
    // A named or numbered sheet narrows the answer; without one the whole
    // workbook is the answer.
    let sheets: Vec<&irondocuments::sheet::Sheet> = match wanted {
        None => workbook.sheets.iter().collect(),
        Some(name) => {
            let by_index = name
                .parse::<usize>()
                .ok()
                .and_then(|index| workbook.sheets.get(index));
            match workbook.sheet_by_name(name).or(by_index) {
                Some(sheet) => vec![sheet],
                None => {
                    return Err(PdfError::MissingPart(format!(
                        "no sheet named or numbered '{name}'"
                    )));
                }
            }
        }
    ...
    let rendered = match format {
        "json" => {
            let selected = irondocuments::sheet::Workbook {
                sheets: sheets.into_iter().cloned().collect(),
        ...
        _ => {
            ...
            for sheet in sheets {
                let (rows, columns) = sheet.extent();
                out.push_str(&format!(
                    "{} — {rows} rows x {columns} columns, {} cells{}\n",
                    sheet.name,
                    sheet.cells.len(),
                    match sheet.hidden {
                        true => " (hidden)",
                ...
                for cell in &sheet.cells {
                    // The formula is what the cell *is*; the value is only the
                    // last thing it evaluated to, so both are shown.
                    match &cell.formula {
                        Some(formula) => out.push_str(&format!(
                            "  {:<8} ={formula}  -> {}\n",
                            cell.reference(),
                            cell.value.to_text()
                        )),
...
/// Recompute the formulas in a spreadsheet and report what does not agree.
///
/// The interesting output is the disagreements: a cell whose stored result no
/// longer follows from its own formula is either stale or edited, and an agent
/// about to quote the number in it should know which. Nothing is written back;
/// the file is audited, not repaired.
#[cfg(feature = "cli")]
fn cmd_recalc(file: &str, format: &str) -> Result<(), PdfError> {
    let workbook = load_workbook(file)?;
    let report = irondocuments::calc::check(&workbook);
    ...
    match format {
        "json" => {
            let json = serde_json::json!({
                ...
                "cycles": report.cycles.iter().map(|(sheet, cell)| serde_json::json!({
                    "sheet": sheet, "cell": cell,
                })).collect::<Vec<_>>(),
                "unparsed": report.unparsed.iter().map(|(sheet, cell, why)| serde_json::json!({
                    "sheet": sheet, "cell": cell, "reason": why.to_string(),
                })).collect::<Vec<_>>(),
            });
            println!(
                "{}",
        ...
        _ => {
            println!("{} formula(s) evaluated", report.evaluated);
            for entry in &report.disagreements {
                println!(
                    "  {}!{}: stored {}, computes to {}",
                    entry.sheet,
                    entry.reference,
                    entry.cached.to_text(),
                    entry.computed.to_text()
                );
            }
            for (sheet, cell) in &report.cycles {
                println!("  {sheet}!{cell}: refers to itself; left as stored");
            }
            for (sheet, cell, why) in &report.unparsed {
                println!("  {sheet}!{cell}: {why}");
            }
            if report.disagreements.is_empty()
                && report.cycles.is_empty()
                && report.unparsed.is_empty()
            {
                println!("Every stored result follows from its formula.");
            }
        }
    }
    Ok(())
...
/// Read a workbook from a spreadsheet file or from ADF.
///
/// Delegates rather than re-deriving: the MCP server answers the same question,
/// and two loaders would be two answers waiting to disagree.
#[cfg(feature = "cli")]
fn load_workbook(file: &str) -> Result<irondocuments::sheet::Workbook, PdfError> {
    let document = load_document(file)?;
    irondocuments::agent_ops::workbook_of(document.data(), &document)
}

/// List the formats this build can read.
#[cfg(feature = "cli")]
fn cmd_formats(format: &str) -> Result<(), PdfError> {
    let formats = irondocuments::describe_formats();
    match format {
        "json" => {
...
fn cmd_describe() -> Result<(), PdfError> {
    println!("{}", irondocuments::describe_ontology());
    Ok(())
}
    ...
    match format {
        "json" => {
            let info = serde_json::json!({
                ...
                "source": "https://github.com/nervosys/IronDocuments",
                "capabilities": [
                    "text_extraction",
                    "unicode_decoding",
                    "markdown_export",
                ...
                "discovery": "Run `irondoc describe` for full JSON-LD ontology with output schemas and workflow templates."
            });
            println!(
                "{}",
        ...
        _ => {
            println!("IronDocuments CLI v1.0.0");
            println!("=====================");
            ...
            println!("Source:   https://github.com/nervosys/IronDocuments");
            println!();
            println!("Tip: Run `irondoc describe` for the full machine-readable ontology.");
        }
    }
...
mod surface_tests {
    ...
    fn ontology_commands() -> Vec<String> {
        let ontology = irondocuments::build_ontology();
        ontology["commands"]
            .as_array()
    ...
    fn the_ontology_describes_every_command_the_cli_offers() {
        ...
        // `irondoc describe` then cannot find capabilities that exist. Asserting
    ...
    fn every_mcp_tool_maps_to_a_real_capability() {
        ...
        for tool in irondocuments::mcp::tool_defs() {
