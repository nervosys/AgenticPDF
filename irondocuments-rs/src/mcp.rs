...
//! them directly. Run with `irondoc mcp`.
        ...
        "initialize" => {
            ...
            Some(ok(
                ...
                json!({
                    "protocolVersion": pv,
                    "capabilities": { "tools": {} },
                    "serverInfo": { "name": "irondocuments", "version": "1.0.0" }
                }),
            ))
        }
...
pub fn tool_defs() -> Vec<Value> {
    vec![
        ...
        json!({
            "name": "sheet",
            "description": "Read a spreadsheet as typed cells rather than as text: the type of every value, the formula behind computed cells alongside the result the source cached, and the number format that says whether a number is a date. Reads .xlsx, and .adf written under the spreadsheet profile.",
            "inputSchema": path_schema(json!({
                "sheet": { "type": "string", "description": "Only this sheet, by name or zero-based index" }
            }), &[])
        }),
        json!({
            "name": "recalc",
            "description": "Recompute a spreadsheet's formulas and report where the stored result no longer follows from the formula beside it. Nothing is written back. A cell that disagrees is stale or hand-edited, which is what to know before quoting the number in it. Cycles and unparseable formulas are reported, not resolved.",
            "inputSchema": path_schema(json!({}), &[])
        }),
        json!({
            "name": "structure",
...
fn run_tool_in(
    ...
    match name {
        ...
        "sheet" => {
            let wanted = args.get("sheet").and_then(|s| s.as_str());
            to_json(&crate::agent_ops::sheet(&data, &document, wanted).map_err(|e| e.to_string())?)
        }
        "recalc" => {
            to_json(&crate::agent_ops::recalc(&data, &document).map_err(|e| e.to_string())?)
        }
