// Headless smoke-test for the IronDocuments WASM module under Node.js
...
async function main() {
  ...
  const wasmPath = path.join(__dirname, 'pkg', 'irondocuments_bg.wasm');
  const jsPath = path.join(__dirname, 'pkg', 'irondocuments.js');
  const samplePath = path.join(__dirname, '..', 'demos', 'sample.pdf');

  console.log('=== IronDocuments WASM Smoke Test ===\n');
  ...
  // The wasm-bindgen module needs a single import namespace: `./irondocuments_bg.js`.
