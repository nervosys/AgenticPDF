...
//!
//! An agent discovers this tool through `irondoc describe` and `irondoc formats`
...
fn the_ontology_lists_exactly_the_formats_the_build_reads() {
    ...
    let supported: BTreeSet<String> = irondocuments::detect::Format::all()
        .iter()
...
fn every_command_either_reads_any_document_or_says_it_is_pdf_only() {
    ...
    for name in commands_from_help() {
        ...
        //
        // The property is that the help tells the truth about what the command
        // accepts, not that there are only two answers. Commands that read one
        // family of formats say so in their own line, and each such phrase is
        // listed here so that adding one is a deliberate act rather than a way
        // to opt out of the check.
        let line = help
            .lines()
            .find(|l| l.trim_start().starts_with(&format!("{name} ")))
            .unwrap_or_default();
        let claims_a_restriction = line.contains("PDF only") || line.contains("spreadsheets only");

        if refused != claims_a_restriction {
            wrong.push(format!(
                "{name}: help {} but the command {}",
                match claims_a_restriction {
                    true => "declares a format restriction",
