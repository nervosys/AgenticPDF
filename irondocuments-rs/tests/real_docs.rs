  ...
  <spine><itemref idref="c1"/><itemref idref="c2"/></spine>
    ...
    irondocuments::testing::build_zip(&[
        ...
        (
            ...
            &irondocuments::testing::png_header(400, 300),
            true,
        ),
    ])
}
...
fn a_typeset_page_renders_the_text_it_reports() {
    ...
    let drawn: Vec<&str> = list
        ...
        .filter_map(|op| match op {
            irondocuments::engine::RenderOp::Text { text, .. } => Some(text.as_str()),
            _ => None,
        })
        .collect();
    assert!(
...
fn typeset_tables_are_recovered_by_the_pdf_table_detector() {
    ...
    let graphics: Vec<_> = (1..=document.page_count())
        .map(|page| {
            ...
            irondocuments::engine::PageGraphics {
                page_number: page,
    ...
    assert!(irondocuments::tables::detect_tables(&graphics, &geometric.pages).is_empty());
...
fn a_damaged_legacy_binary_fails_as_a_container_not_as_a_pdf() {
    ...
    assert_eq!(
        irondocuments::detect::detect(&ole2, None).unwrap(),
        Format::Doc
    );
    assert!(matches!(
        Document::open(&ole2),
